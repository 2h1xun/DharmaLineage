...
function nodeActive(a) {
	...
	if (groupByDirection) {
        ...
        // Incoming -> 스승
        f.push("<h2>스승 (" + size + ")</h2>"); 
        (size>0)? f=f.concat(createList(incoming)) : f.push("스승 정보 없음<br>");
        size=Object.size(outgoing);
        // Outgoing -> 제자
        f.push("<h2>제자 (" + size + ")</h2>"); 
        (size>0)? f=f.concat(createList(outgoing)) : f.push("제자 정보 없음<br>");
    }
    ...
    if (f.attributes) {
        ...
        temp_array = [];
        g = 0;
        var targetAttributes = ["hanzi", "birthdate", "deathdate", "originannotation", "transannotation", "note"];
        var labelMap = {
            "hanzi": "한자",
            "birthdate": "생년",
            "deathdate": "몰년",
            "originannotation": "주석(원문)",
            "transannotation": "주석(번역)",
            "note": "비고"
        };
        for (var i = 0; i < targetAttributes.length; i++) {
            var attrKey = targetAttributes[i];
            
            if (f.attributes && f.attributes[attrKey]) {
                var d = f.attributes[attrKey];
                
                var displayLabel = labelMap[attrKey] ? labelMap[attrKey] : attrKey;
                var h = '<span><strong>' + displayLabel + ':</strong> ' + d + '</span><br/>';
                e.push(h);
            }
        }
